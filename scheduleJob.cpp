...
int main()
{
	...
	while(!feof(fp))
	{
		...
		if (incomingJob.size()>=SYSTEM_END_TIME)
		{
			break;
		}
	}
	...
	for (int i=0;i<greenEnergy.size();++i)
	{
		...
		fputs(wbuf,wp);
		fputs(",",wp);
		sprintf(wbuf,"%f",greenEnergy[i].power);//greenPower
		fputs(wbuf,wp);
		fputs(",",wp);
		sprintf(wbuf,"%f",gridPower[i].power);//greenPower
		fputs(wbuf,wp);
		fputs("\n",wp);
	}
	fclose(wp);
